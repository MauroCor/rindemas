    ...
    <div className="min-h-screen py-4" style={{background:'#111827', color:'#F3F4F6'}}>
      ...
      <CarouselComponent
        ...
        renderItem={(monthData) => (
          <div className={`w-60 rounded-xl p-4 shadow-lg text-center border ${isCurrentYearMonth(monthData.date) ? 'border-teal-500' : 'border-gray-700'}`} style={{background:'#1F2937', borderColor: isCurrentYearMonth(monthData.date) ? '#16A085' : '#374151'}}>
            <h3 className="font-bold text-2xl mb-4">{getMonthName(monthData.date)}</h3>
            <div className="mb-3">
              <label>Balance</label>
              <div>
                <label className='text-2xl font-bold' style={{color:'#16A085'}}>{formatPrice((monthData.balance?.total || 0) + (monthData.liquidez?.total || 0), 'ARS')}</label>
              </div>
            </div>
